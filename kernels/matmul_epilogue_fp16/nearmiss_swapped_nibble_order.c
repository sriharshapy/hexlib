 ...
 *
 * THE MISTAKE: swapping which nibble of each packed byte maps to which
 * dequantized index. The correct mapping (kernel_api.h SPEC SOURCE 3,
 * adapted from llama.cpp's dequantize_row_q4_0): low nibble of byte j ->
 * index j, high nibble of byte j -> index j+16. This file writes it the
 * other way: HIGH nibble -> index j, LOW nibble -> index j+16.
 *
 * WHY ANYONE WOULD WRITE IT. "byte j holds elements 2j and 2j+1" is the more
 * common packing convention in adjacent-pair nibble formats, and swapping
 * which nibble is "first" inside that assumption is an easy transcription
 * slip that still compiles, still produces a same-shape output, and still
 * looks locally sensible (the shift amount 4 and mask 0x0F are still exactly
 * right -- only which RESULT INDEX each one is assigned to is wrong).
 *
 * WHY THIS IS EASY TO CATCH HERE (not quiet like the fp16/gelu near-misses).
 * Unless a block's 32 target values happen to be symmetric under swapping
 * (j, j+16), every block's dequantized values land at different indices
 * entirely -- not a rounding-sized perturbation but a full permutation of
 * which weight multiplies which activation term. Measured in Python against
 * this exact algorithm on ordinary formula-generated (non-adversarial,
 * mixed-sign) weight data at this kernel's own test shapes: 85% of ALL
 * output elements disagree with the correct reference, max relative error
 * in the tens of millions of percent (a wrong-index sum is not a small
 * number near a right one). No dedicated adversarial cell needed.
 */
...
/* WRONG: nibble roles swapped. HIGH nibble -> index j, LOW nibble -> j+16. */
static inline void mm_dequant_block(const unsigned char *blk, hexlib_hf *out32) {
    ...
    for (int j = 0; j < 16; ++j) {
        ...
        out32[j]      = (hexlib_hf) (df * (float) hi);  /* swapped */
        out32[16 + j] = (hexlib_hf) (df * (float) lo);  /* swapped */
    }
}
