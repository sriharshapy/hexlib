 ...
 *
 * THE MISTAKE: dequantizing output-column-block bb's 32 codes with the
 * SCALE from output-column-block (bb-1), instead of block bb's own scale.
 * The codes read are still correct for block bb; only the 2-byte fp16
 * scale prefix comes from the wrong 18-byte block.
 *
 * WHY ANYONE WOULD WRITE IT. Pointer arithmetic over a stream of same-sized
 * 18-byte records is exactly the kind of code where an index that should
 * track the current iteration silently tracks the PREVIOUS one instead --
 * an off-by-one in which block's base address feeds the scale read, while
 * the codes pointer (computed separately, correctly) does not share the
 * mistake. Nothing about the surrounding loop looks wrong; the block being
 * read is still 18 bytes, still at a valid offset within the same row.
 *
 * WHY THIS IS EASY TO CATCH HERE. This kernel's own blocks (kernel_api.h's
 * layout decision) are quantized independently per (row k, output-block bb)
 * with amax-derived scales, so adjacent blocks along a row generally have
 * DIFFERENT scales (harness.c's weight-generating formula varies with `bb`
 * specifically to make sure of this). Applying the wrong scale multiplies
 * every one of that block's 32 dequantized values by (right_scale /
 * wrong_scale) -- a systematic, block-wide distortion, not a rounding
 * perturbation. Measured in Python against this exact algorithm on ordinary
 * formula-generated data at this kernel's own test shapes: 61% of ALL
 * output elements disagree with the correct reference, max relative error
 * over 100000% (block 0 of every row, which has no "previous" block in this
 * implementation's wrap-around indexing, borrows the LAST block's scale --
 * still a real, present mismatch, not a skipped case). No dedicated
 * adversarial cell needed.
 */
...
/* WRONG: scale comes from `scale_blk`, codes come from `codes_blk` -- the
 * two pointers are the SAME block in the correct kernel, but the caller
 * below passes them one block apart. */
static inline void mm_dequant_block_wrong_scale(const unsigned char *scale_blk,
                                                const unsigned char *codes_blk,
                                                hexlib_hf *out32) {
    __fp16 d;
    memcpy(&d, scale_blk, sizeof(d));
    const float df = (float) d;
    const unsigned char *qs = codes_blk + 2;
    for (int j = 0; j < 16; ++j) {
...
void matmul_epilogue_fp16(const hexlib_hf *a, const unsigned char *w,
    ...
    for (int m = 0; m < M; ++m) {
        ...
        for (int bb = 0; bb < nblocks; ++bb) {
            ...
            /* WRONG: the scale is read from the PREVIOUS block (wrapping
             * around for bb==0), while the codes still come from bb. */
            const int scale_bb = (bb - 1 + nblocks) % nblocks;
            const long scale_off = (long) scale_bb * MM_Q4_0_BLOCK_BYTES;
            HVX_Vector acc_lo = Q6_V_vzero();

            for (int k = 0; k < K; ++k) {
                const unsigned char *codes_blk = w + (long) k * row_stride + bb_off;
                const unsigned char *scale_blk = w + (long) k * row_stride + scale_off;
                mm_dequant_block_wrong_scale(scale_blk, codes_blk, wbuf);
